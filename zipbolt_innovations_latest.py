...
# Add/modify devices as needed
DEVICE_TABLES = ["device2","device3","device3","device4" , "device5", "device6", "device7", "device8"]  # Add/modify devices as needed
...
# After API data loading block — ADD THIS:

# ===========================================
# LOAD AND MERGE MULTIPLE LOCAL CSV FILES TOO
# ===========================================
...
CSV_FILES = [
    ...
    "d7.csv",
    "d8.csv"
]  # Add all CSV file names here

print("\n📌 Loading additional CSV device data...")
csv_loaded = 0
...
for csv_path in CSV_FILES:
    if os.path.exists(csv_path):
        ...
        df_csv['device_id'] = df_csv.get('device_id', device_id)
...
if csv_loaded > 0:
    print(f"✓ Loaded {csv_loaded} CSV files and merged with API data")

# Re-combine API + CSV data
df = pd.concat(all_dfs, ignore_index=True)

print(f"{'='*80}")
print("📦 FINAL DATA MERGE COMPLETE")
print(f"{'='*80}")
print(f"Total final rows: {len(df):,}")
print(f"Devices included: {', '.join(set(device_names))}")
print(f"Memory usage: {df.memory_usage(deep=True).sum() / 1024**2:.2f} MB")
